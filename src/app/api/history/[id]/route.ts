...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (patch.profile) updates.profile = patch.profile;
    ...
    if (patch.graph !== undefined) {
      if (patch.graph === null) {
        await deleteObjectIfExists(item.graphKey);
        updates.graphKey = undefined;
      } else {
        const graphKey = item.graphKey ?? `history/${uid}/${id}/graph.json`;
        await putJson(graphKey, patch.graph);
        updates.graphKey = graphKey;
      }
    }

    if (patch.results !== undefined) {
      if (patch.results === null) {
        await deleteObjectIfExists(item.resultsKey);
        updates.resultsKey = undefined;
      } else {
        const resultsKey = item.resultsKey ?? `history/${uid}/${id}/results.json`;
        await putJson(resultsKey, patch.results);
        updates.resultsKey = resultsKey;
      }
    }
  ...
  try {
    ...
    if (current.Item) {
      await Promise.all([
        ...
        deleteObjectIfExists((current.Item as any).graphKey),
        deleteObjectIfExists((current.Item as any).resultsKey),
      ]);
    }
